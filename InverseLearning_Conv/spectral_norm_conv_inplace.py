...
class SpectralNormConv(object):
    ...
    def compute_weight(self, module, do_power_iteration):
        ...
        factorReverse = torch.max(torch.ones(1).to(weight.device), sigma / self.coeff)  # coeff=.97
        # factorReverse = torch.max(torch.ones(1).to(weight.device), sigma / 1) # make lipschitz_constant = 0.99 for layer
        ...
        if resnet:
            weight = weight
            # lipschitz_constant = sigma
        else:
            # # rescaling
            weight = weight / (factorReverse + 1e-5)  # for stability
            # lipschitz_constant = sigma / (factorReverse + 1e-5)  # linear change with weights change

        # print("sigma | factorReverse -> Lips: {} | {} -> {}".format(sigma.item(), factorReverse.item(), lipschitz_constant.item()))
        # lipschitz_constant: lipschitz_constant after updating weights
        # sigma:  lipschitz_constant before updating weights
        return weight
