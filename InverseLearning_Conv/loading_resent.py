...
parser.add_argument('--nBlocks', nargs='+', type=int, default=[4, 4, 4])   # 4, 4, 4   / 1, 0, 0
parser.add_argument('--nStrides', nargs='+', type=int, default=[1, 1, 1])   # 1, 2, 2  / 1, 0, 0
parser.add_argument('--nChannels', nargs='+', type=int, default=[12, 12, 12])  #16, 64, 256
...
#         # Note: torch.lu is deprecated; use torch.linalg.lu_factor instead
#         # However, torch.linalg.lu_factor is not differentiable, so for this example, we'll use scipy
#         W_numpy = W.cpu().numpy()
#         P, L, U = scipy.linalg.lu(W_numpy)
#         L = torch.from_numpy(L).to(W.device).float()
#         U = torch.from_numpy(U).to(W.device).float()

#         # Create two Linear layers
#         self.linear_U = nn.Linear(U.shape[1], U.shape[0], bias=False)
#         self.linear_L = nn.Linear(L.shape[1], L.shape[0], bias=(b is not None))

#         # Assign the decomposed weights
#         self.linear_U.weight.data = U
#         self.linear_L.weight.data = L

#         # Assign the bias to the second linear layer
#         if b is not None:
#             self.linear_L.bias.data = b
#         else:
#             self.linear_L.bias = None
...
#         # Perform LU decomposition on W using scipy's lu
#         W_numpy = W.cpu().numpy()
#         P, L, U = scipy.linalg.lu(W_numpy)
        
#         # Convert L and U back to torch tensors
#         L = torch.from_numpy(L).to(W.device).float()
#         U = torch.from_numpy(U).to(W.device).float()
...
#         # Store the permutation matrix P as a torch tensor
#         self.permutation = torch.from_numpy(P).to(W.device).float()

#     def forward(self, x):
#         # Apply the permutation to the input (to account for P)
#         x = torch.matmul(self.permutation, x)
...
#         out = torch.matmul(self.P, out.T).T
#         print("out", x.shape)
...
#     def forward(self, x):
#         print("x", x.shape)
#         # x has shape [batch_size, L, in_features]
#         # Apply the first linear layer (U)
#         # We need to reshape x to combine batch_size and L for the linear layer
#         batch_size, L, in_features = x.shape

#         # Reshape x to [batch_size * L, in_features]
#         # x_reshaped = x.view(-1, in_features)
#         x_reshaped = x.reshape(-1, in_features)

#         # Apply the first linear layer
#         out = self.linear_U(x_reshaped)  # Shape: [batch_size * L, mid_features]

#         # Apply the second linear layer
#         out = self.linear_L(out)  # Shape: [batch_size * L, out_features]

#         # Apply the permutation matrix P
#         # P has shape [out_features, out_features], we can apply it directly
#         out = torch.matmul(out, self.P.T)  # Shape: [batch_size * L, out_features]

#         # Add the original bias
#         if self.bias is not None:
#             out = out + self.bias

#         # Reshape back to [batch_size, L, out_features]
#         # out = out.view(batch_size, L, -1)
#         out = out.reshape(batch_size, L, -1)
#         return out
