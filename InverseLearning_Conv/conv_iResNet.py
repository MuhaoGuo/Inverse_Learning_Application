...
class conv_iResNet(nn.Module):
    def __init__(self, in_shape, nBlocks, nStrides, nChannels, init_ds=2, inj_pad=0,
                 ...
                 n_power_iter=5,
                 block=conv_iresnet_block,  # todo conv_iresnet block layer
...
class conv_resnet_block(nn.Module):
    def __init__(self, in_shape, int_ch, numTraceSamples=0, numSeriesTerms=0,
        ...
        in_ch = in_ch * stride ** 2
        kernel_size1 = 3  # kernel size for first conv  3
        layers.append(
            nn.Conv2d(in_ch, int_ch, kernel_size=kernel_size1, stride=1, padding=1))   # todo padding =1
        layers.append(nonlin())
        kernel_size2 = 1  # kernel size for second conv
        layers.append(nn.Conv2d(int_ch, int_ch, kernel_size=kernel_size2, padding=0))
        layers.append(nonlin())
        kernel_size3 = 3 # kernel size for third conv 3
        layers.append(nn.Conv2d(int_ch, in_ch, kernel_size=kernel_size3, padding=1)) # todo padding =1
        self.bottleneck_block = nn.Sequential(*layers)
        if actnorm:
            self.actnorm = ActNorm2D(in_ch)
        else:
...
########  specific resnet and resnet_block ###########
class conv_resnet_block_specific(nn.Module):
    def __init__(self, in_shape, int_ch, numTraceSamples=0, numSeriesTerms=0,
                 stride=1, coeff=.97, input_nonlin=True,
                 actnorm=True, n_power_iter=5, nonlin="elu"):
        """
        ...
        :param nonlin: the nonlinearity to use
        """
        super(conv_resnet_block_specific, self).__init__()
        assert stride in (1, 2)
        self.stride = stride
        self.squeeze = Squeeze(stride)
        self.coeff = coeff
    ...
    def forward(self, x, ignore_logdet=False):
        ...
        if self.stride == 2:
            x = self.squeeze.forward(x)
            # print("do self.stride == 2, squeeze")
        # x = self.squeeze.forward(x)   # todo must squeeze
        ...
        # add residual to output
        # print("one block out Fx", Fx.shape)
        y = Fx + x
        return y, trace + an_logdet
...
class conv_ResNet_specific(nn.Module):
    def __init__(self, in_shape, nBlocks, nStrides, nChannels, init_ds=2, inj_pad=0,
                 coeff=.9, density_estimation=False, nClasses=None,
                 numTraceSamples=1, numSeriesTerms=1,
                 n_power_iter=5,
                 block = conv_resnet_block_specific,  # todo conv_resnet block layer
                 actnorm=True, learn_prior=True,
                 nonlin="relu"):
        super(conv_ResNet_specific, self).__init__()
        assert len(nBlocks) == len(nStrides) == len(nChannels)
        assert init_ds in (1, 2), "can only squeeze by 2"
        self.init_ds = init_ds
        self.ipad = inj_pad
