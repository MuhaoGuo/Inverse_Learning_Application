...
class SpectralNormMLP(object):
        ...
        if do_power_iteration:
            with torch.no_grad():
                for _ in range(self.n_power_iterations):
                    ...
                    v = normalize(torch.mv(weight.t(), u), dim=0, eps=self.eps)
                    # Power iteration for u
                    u = normalize(torch.mv(weight, v), dim=0, eps=self.eps)
    ...
    def apply(module, coeff, input_dim, name, n_power_iterations, eps):
        ...
        with torch.no_grad():
             u = normalize(weight.data.new(weight.data.size(0)).normal_(0, 1), dim=0, eps=fn.eps)
...
class SpectralNormConvLoadStateDictPreHook(object):
    ...
    def __call__(self, state_dict, prefix, local_metadata, strict,
        ...
        fn = self.fn
        version = local_metadata.get('spectral_norm_mlp', {}).get(fn.name + '.version', None)
...
class SpectralNormConvStateDictHook(object):
    ...
    def __call__(self, module, state_dict, prefix, local_metadata):
        if 'spectral_norm_mlp' not in local_metadata:
            local_metadata['spectral_norm_mlp'] = {}
        key = self.fn.name + '.version'
        if key in local_metadata['spectral_norm_mlp']:
            raise RuntimeError("Unexpected key in metadata['spectral_norm_mlp']: {}".format(key))
        local_metadata['spectral_norm_mlp'][key] = self.fn._version


def spectral_norm_mlp(module, coeff, input_dim, name='weight', n_power_iterations=1, eps=1e-12):
    ...
    Example::
        ...
        torch.Size([20])
    """
    SpectralNormMLP.apply(module, coeff, name, n_power_iterations, eps)
    return module


def remove_spectral_norm_mlp(module, name='weight'):
    ...
    """
    for k, hook in module._forward_pre_hooks.items():
        if isinstance(hook, SpectralNormMLP) and hook.name == name:
            hook.remove(module)
