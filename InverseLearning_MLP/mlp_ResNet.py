...
class mlp_resnet_block(nn.Module):
    def __init__(self, in_shape, int_ch, numTraceSamples=0, numSeriesTerms=0, coeff=.97, input_nonlin=True,
        ...
        """
        super(mlp_resnet_block, self).__init__()
        ...
        layers.append(nn.Linear(in_features, in_features))  # Add the Linear layer
        layers.append(nonlin())
        #kernel_size2 = 1  # kernel size for second conv
        # layers.append(self._wrapper_spectral_norm(nn.Linear(in_ch, in_features), in_features))
        layers.append(nn.Linear(in_features, in_features))
        layers.append(nonlin())
        #kernel_size3 = 3  # kernel size for third conv
        #layers.append(self._wrapper_spectral_norm(nn.Linear(in_features, in_ch), in_features))
        layers.append(nn.Linear(in_features, in_features))
        # layers.append(nonlin())
        #kernel_size3 = 3  # kernel size for third conv
        #layers.append(self._wrapper_spectral_norm(nn.Linear(in_ch, in_features), in_features))
...
class mlp_ResNet(nn.Module):
    def __init__(self, in_shape, nBlocks, nStrides, nChannels, init_ds=2, inj_pad=0,
                 ...
                 n_power_iter=5,
                 block=mlp_resnet_block,  # todo conv_iresnet block layer
...
if __name__ == "__main__":
    ...
    resnet = mlp_ResNet(in_shape[1:], [4, 4, 4], [1, 2, 2], [32, 32, 32],
